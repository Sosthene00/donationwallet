...
use sp_backend::{silentpayments::receiving::Receiver, spclient::SpWallet};

use sp_backend::spclient::{OutputSpendStatus, OwnedOutput, SpClient};
...
pub fn scan_blocks(
    ...
    let sp_client = sp_wallet.get_mut_client();
    let mut outputs = sp_wallet.get_mut_outputs();
    ...
    for n in start..=end {
        if n % 10 == 0 || n == end {
            send_scan_progress(crate::stream::ScanProgress {
                start,
                current: n,
                end,
            });
        ...
        let owned_spks: Vec<Vec<u8>> = outputs
            .to_spendable_list()
            .iter()
            .map(|(_, x)| {
        ...
        if matched {
            ...
            if !owned.is_empty() {
                outputs.extend_from(owned);
                send_amount_update(outputs.get_balance().to_sat());
            ...
            let inputs_found = scan_block_inputs(&mut outputs, blk.txdata)?;
...
fn get_script_to_secret_map(
    ...
    for shared_secret in shared_secrets {
        let script_bytes = sp_receiver.get_spks_from_shared_secret(&shared_secret)?;
...
) -> Result<HashMap<bitcoin::OutPoint, OwnedOutput>> {
...
fn scan_block_inputs(
    ...
    for tx in txdata {
        for input in tx.input {
            let prevout = bitcoin::OutPoint::from_str(&input.previous_output.to_string())?;

            if outputs.get_outpoint(prevout).is_ok() {
                found.push(prevout);
            }
        }
    }
...
fn deserialize_transaction(tx: &str) -> Result<Transaction> {
    Ok(Transaction::deserialize(&Vec::from_hex(tx)?)?)
}

pub fn broadcast_transaction(mut handle: Handle<Waker>, tx_str: String) -> Result<String> {
    let tx = deserialize_transaction(&tx_str)?;
